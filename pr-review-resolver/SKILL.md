...
- **Integração com GitHub/GitLab:** Sempre utilize as ferramentas oficiais de linha de comando para interagir com a plataforma:
  - Para GitHub, utilize o GitHub CLI (`gh`). Ex: `gh pr view`, `gh pr review`.
  - Para GitLab, utilize o GitLab CLI (`glab`). Ex: `glab mr view`, `glab mr note`.
- **Automação:** Use essas ferramentas não apenas para ler as revisões, mas ativamente para responder aos comentários diretamente do terminal.
...
1. **Extração e Organização:** Obtenha os comentários de revisão pendentes utilizando a CLI adequada (`gh` ou `glab`) e organize-os como uma lista estruturada de tarefas para analisar e resolver cada um.
2. **Execução das Tarefas:** Para cada revisão de código listada:
   ...
   - **Caminho 1 (É Coerente):**
     ...
     5. Responda ao comentário original na plataforma utilizando o `gh` ou `glab` para explicar o que foi feito.
   - **Caminho 2 (Não é Coerente):**
     1. Não faça nenhuma alteração no código relacionada a esse comentário.
     2. Adicione uma resposta ao review usando `gh` ou `glab` explicando técnica e educadamente por que a sugestão não é aplicável ou coerente.
3. **Verificação Final:** Assegure-se de que todos os comentários foram respondidos com código atualizado ou com uma justificativa diretamente via CLI da plataforma.
